...
`aside.side` (ops) болон `nav.tabs` (supplier ≥900) `sidebar` класс нэмнэ. Sticky 100dvh, 240, `--surface`, border-right, 20 12 16. `.brand` — Basu 18/600 `--ink` + Ops/Нийлүүлэгч 13/500 `--ink-3`. `nav`/`.tabs` flex column, `overflow-y:auto`, `flex:1` — Гарах хэзээ ч тасрахгүй. `.grp` eyebrow 16 10 6. `button[data-tab]` — 36px, 14/500 `--ink-2`, svg 18 `--ink-3`, hover `--surface-2`, `[data-on]` accent-soft + accent-ink 600 + svg accent, `aria-current=page` (JS). `.n[data-badge]` — count (neutral; `[data-hot]` accent). `.conn` цэг + үг (supplier-т `#conn`-ийн ширээний хуулбар — id давхардуулахгүй, `.conn` класс, `querySelectorAll`). `.me` hairline дээр, нэр 13/500, роль·утас mono 11.5; `.out` quiet 36 → hover stop-soft, төхөөрөмж дээр хоёр алхамт баталгаажуулалт. ≤900px sidebar → `.tabbar` (шошготой 4–5 tab + «Бусад» sheet; бүх `button[data-tab]` DOM-д үлдэнэ).
...
Бүх хуудсанд: (1) хуудас бүр яг нэг `<script type="module">…</script>` (атрибутгүй — тест `/<script type="module">([\s\S]*?)<\/script>/` эхнийхийг авна); import `import { … } from '/api.js';` / `'/mapStyle.js';` хэлбэрээр мөрийн төгсгөлд (regex `/^\s*import[\s\S]*?from\s*'\/[\w.]+';?$/m`); хоёр дахь inline script hash-лагдана, тест ажиллуулахгүй. (2) `src/api/hardening.ts` inline script бүрийг сервер эхлэхэд SHA-256 хийнэ — JS өөрчлөгдвөл restart; `hardening.test` хуудас бүрт нэг hash. (3) `localStorage` түлхүүр `basu.guest`, `basu.device` (`all-kitchens` sentinel), `basu.supplier` (`all-suppliers`), `basu.ops`, `basu.ops.tab/.period/.money/.notify`; `sessionStorage ops.code.<id>`. (4) `#toast` (api.js үүсгэнэ, `role=status`, `data-show`, `data-kind=bad`) — тест timeout дээр уншина. (5) `html.in-shell` (api.js shell дотор). (6) `.clockbar` markup: `.lab` «Демо цаг», `#clock-now.now.mn`, `button[data-to|data-advance|data-tick][data-hot]`. (7) Бүх монгол копи үг үсгээрээ хэвээр; `№`, `·`, `₮` (`mnt()`), `—`.
